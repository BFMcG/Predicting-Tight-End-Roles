...
The reason for splitting data by previous 32 team testing data is because it allows us to compare the overall model to 32 team models more accurately.

Every test observation the TE overall model uses will be the same for the 32 team models, and we'll be able to see the difference between the model performances.
...
```{r, eval=FALSE}
save(final_rf_TE, final_rf_test, file = "file/TE fit.rda")
```
...
If you choose to save the team fitted models
...
If you choose to save TE vs Team augmentation
...
```
**\$** Graphing Denvers TE roles (looking at Eric Tomlinson)
```{r}
